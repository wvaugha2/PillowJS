...
class HttpHeaders {
      ...
      for (let i = 0; i < headersKeys.length; i++) {
        ...
        if (typeof key === 'string') {
          ...
          if (typeof value === 'string' || isArrayOfStr(value)) {
            ...
            throw new Error(`The provided value for header ${key} must be a string or an array of strings: ${value}`)
          }
        } else {
          throw new Error(`The following header key must be a string: ${key}`);
        }
      }
    }
  }
   ...
   * @returns {string} the value of the header, or null if the header does not exist
   */
  getHeader(key) {
